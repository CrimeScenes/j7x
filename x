...
local fovRadius = 15  -- FOV circle radius for the triggerbot
...
local function isWithinFovCircle(targetPosition)
...
-- Get body parts of the target character
...
local function getPredictedPosition(target)
    ...
    if humanoid and humanoid.Health > 0 then
        ...
        if rootPart then
            ...
            local predictedPos = rootPart.Position + velocity * 0.1  -- Predicted position after 0.1 seconds
            return predictedPos
        end
    end
...
-- Check if the player is knocked (Health <= 1)
...
-- Check if the player is ignoring the knife
...
-- Trigger the bot action, predicting the target's position and shooting
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    local predictedPos = getPredictedPosition(TargetPlayer)  -- Get predicted target position
                    local screenPos, onScreen = Camera:WorldToViewportPoint(predictedPos)
                    
                    -- If target is within FOV and on screen
                    if onScreen and isWithinFovCircle(predictedPos) then
                        syncBoxWithTarget(screenPos)
                        
                        -- If cooldown passed, perform the click
...
-- Keybind listener to toggle the triggerbot
...
-- RenderStepped listener to trigger the bot when toggled
